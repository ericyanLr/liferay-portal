		...
		try (LogCapture logCapture = LoggerTestUtil.configureLog4JLogger(
			...
			for (Portlet portlet : portletList) {
				...
				if (categoryName == null) {
					...
					for (int i = 0; i < nestedCategoryNames.length; i++) {
						String nestedCategoryName = nestedCategoryNames[i];

						path += StringPool.DOUBLE_SLASH + nestedCategoryName;
						portletCategory = portletCategory.getCategory(
							nestedCategoryName);

						Assert.assertEquals(
							nestedCategoryName, portletCategory.getName());
						Assert.assertEquals(path, portletCategory.getPath());

						if (i == (nestedCategoryNames.length - 1)) {
							Set<String> portletIds =
								portletCategory.getPortletIds();
